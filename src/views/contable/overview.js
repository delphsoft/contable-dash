...
export function renderOverview(el) {
  ...
  el.innerHTML=`
    <div class="hero-card">
      ...
      <div class="hero-row">
        ...
        <span class="hero-delta d-up"><i class="ti ti-trending-up" style="font-size:12px"></i>5 meses</span>
      </div>
    ...
    <div class="grid-3">
      <div class="kpi">
        ...
        <div class="kpi-val up">${fmtM(tB)}</div>
        <div class="kpi-delta up"><i class="ti ti-arrow-up-right" style="font-size:13px"></i>${fmtPct(tB/tV*100)}</div>
      </div>
      <div class="kpi">
        <div class="kpi-lbl">Resultado neto</div>
        <div class="kpi-val dn">${fmtM(tN)}</div>
        <div class="kpi-delta dn"><i class="ti ti-arrow-down-right" style="font-size:13px"></i>${fmtPct(tN/tV*100)}</div>
      </div>
      <div class="kpi">
        <div class="kpi-lbl">CMV / Ventas</div>
        <div class="kpi-val" style="color:var(--amber)">84.5%</div>
        <div class="kpi-delta dn"><i class="ti ti-arrow-down-right" style="font-size:13px"></i>obj. ≤70%</div>
      </div>
    </div>
      ...
      <div class="chart-hd">
        <div>
          ...
          <div class="chart-sub-row">
            <span class="chart-big">${fmtM(tV)}</span>
            <span class="hero-delta d-up" style="font-size:11px"><i class="ti ti-trending-up" style="font-size:11px"></i>acumulado</span>
          </div>
        </div>
        <div class="legend">
          <div class="legend-item"><div class="legend-dot" style="background:#378ADD"></div>Ventas</div>
          <div class="legend-item">
            <span style="display:inline-block;width:14px;border-top:2px dashed #f05a5a;vertical-align:middle;margin-right:5px"></span>CMV
          </div>
        </div>
      </div>
      <div class="chart-wrap h220"><canvas id="ov1" role="img" aria-label="Ventas y CMV mensual oct-25 a feb-26"></canvas></div>
      <div style="display:flex;justify-content:center;margin-top:14px">
        <div style="width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,0.07);display:flex;align-items:center;justify-content:center;cursor:pointer">
          <i class="ti ti-arrow-down" style="font-size:15px;color:var(--txt2)"></i>
        </div>
      </div>
    </div>
      ...
      <div class="chart-card" style="margin-bottom:0">
        <div class="chart-hd">
          ...
          <div class="legend">
            ...
            <div class="legend-item">
              <span style="display:inline-block;width:12px;border-top:2px dashed #f05a5a;vertical-align:middle;margin-right:5px"></span>Neto
            </div>
          </div>
        </div>
  ...
  new Chart(document.getElementById('ov1'),{
    ...
    data:{labels:MONTHS,datasets:[
      ...
      {label:'CMV',type:'line',data:CMV,borderColor:'#f05a5a',backgroundColor:'transparent',tension:0.35,pointRadius:5,pointBackgroundColor:'#f05a5a',borderWidth:2,borderDash:[6,4],order:1},
    ]},
    options:base()
  })
  ...
  new Chart(document.getElementById('ov3'),{
    ...
    data:{labels:MONTHS,datasets:[
      ...
      {data:MN_PCT,borderColor:'#f05a5a',fill:false,tension:0.35,pointRadius:3,borderWidth:2,borderDash:[5,3]},
    ]},
    options:base(v=>v+'%')
  })
}
